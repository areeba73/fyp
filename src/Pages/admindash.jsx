...
const AdminDashboard = () => {
  ...
  const [users, setUsers] = useState([
    { id: 1, name: "David Miller", email: "david@example.com", joined: "12 Apr 2024", mood: "😊", status: "Active" },
    { id: 2, name: "Sarah Khan", email: "sarah@example.com", joined: "10 Apr 2024", mood: "😐", status: "Active" },
    { id: 3, name: "John Wick", email: "john@example.com", joined: "08 Apr 2024", mood: "😡", status: "Inactive" },
  ]);

  const [doctors, setDoctors] = useState([
    { id: 101, name: "Dr. Aryan Sharma", email: "aryan@med.com", exp: "8 Years", spec: "Psychiatrist", status: "Available" },
    { id: 102, name: "Dr. Zainab Ali", email: "zainab@med.com", exp: "5 Years", spec: "Therapist", status: "On Leave" },
  ]);

  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to remove this record?")) {
      if (activeTab === 'users') setUsers(users.filter(u => u.id !== id));
      else setDoctors(doctors.filter(d => d.id !== id));
    }
  };
  ...
  return (
    <div className="min-h-screen relative font-sans text-blue-900 overflow-x-hidden">
      ...
      <div className="pt-8 px-4 md:px-10 lg:px-20 max-w-[1600px] mx-auto">
        <Link to="/" className="flex items-center z-[1100] w-fit">
          <img 
            src={logo} 
            alt="EmoTrack Logo" 
            className="h-[50px] md:h-[55px] transition-all duration-300" 
          />
        </Link>
      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white/50 p-6 rounded-[2rem] border border-white shadow-lg flex items-center gap-4">
                     ...
                     <div>
                       ...
                       <h4 className="text-2xl font-black text-blue">{users.length}</h4>
                     </div>
                  </div>
                  <div className="bg-white/10 p-6 rounded-[2rem] border border-white shadow-lg flex items-center gap-4">
                     ...
                     <div>
                       ...
                       <h4 className="text-2xl font-black text-blue">{doctors.length}</h4>
                     </div>
                  </div>
                </div>
                ...
                <div className="bg-white/40 rounded-[2.5rem] shadow-2xl border border-white overflow-hidden">
                  <div className="flex flex-col sm:flex-row items-center justify-between p-6 border-b border-blue-50 bg-white/20 gap-4">
                    <div className="flex gap-2">
                      <button onClick={() => {setActiveTab('users'); setSearchQuery('')}} className={`px-5 py-2 rounded-xl font-bold text-xs transition-all ${activeTab === 'users' ? 'bg-[#2F357D] text-white shadow-md' : 'text-blue hover:bg-blue-50'}`}>Users</button>
                      <button onClick={() => {setActiveTab('doctors'); setSearchQuery('')}} className={`px-5 py-2 rounded-xl font-bold text-xs transition-all ${activeTab === 'doctors' ? 'bg-[#2F357D] text-white shadow-md' : 'text-blue hover:bg-blue-50'}`}>Doctors</button>
                    </div>
                    <input 
                      type="text" 
                        ...
                        {filteredData.map((item) => (
                          <tr key={item.id} className="border-b border-blue-50/50 hover:bg-blue-50/20 transition-all">
                            ...
                            <td className="py-4 px-4 text-center">
                              <button onClick={() => handleDelete(item.id)} className="text-[10px] font-bold text-blue hover:underline">Remove</button>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                  ...
                  <div className="relative">
                    ...
                    <div className="relative">
                      <input 
                        ...
                        className="w-full mt-1 px-5 py-4 bg-blue-50 border border-blue-100 rounded-2xl font-bold text-blue-950 focus:outline-none focus:ring-2 focus:ring-blue-400 transition-all"
                      />
                      <button 
                        type="button"
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                    <div className="bg-blue-400 h-full w-[99%] transition-all duration-1000"></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden">
                    <div className="bg-emerald-400 h-full w-[12%] transition-all duration-1000"></div>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                    <p className="text-[10px] text-blue-200/50 uppercase font-bold">Uptime</p>
                    <p className="text-sm font-black">24d 14h</p>
                  </div>
                  <div className="bg-white/5 p-4 rounded-2xl border border-white/10 text-center">
                    <p className="text-[10px] text-blue-200/50 uppercase font-bold">Response</p>
                    <p className="text-sm font-black">124ms</p>
                  </div>
                </div>
              </div>
            </div>

            <div className="bg-white/40 backdrop-blur-md rounded-[2.2rem] p-3 border border-white shadow-lg">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-7 h-7 bg-[#2F357D] rounded-lg flex items-center justify-center text-white text-[10px] shadow-sm">
                  📢
                </div>
                <h4 className="font-black text-blue text-[10px] uppercase tracking-widest">Quick Notice</h4>
              </div>
              <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-100/50">
                <p className="text-[10px] leading-relaxed text-blue font-bold italic">
                  "All systems are currently optimized. Daily security protocols have been successfully executed."
                </p>
              </div>
              <div className="flex items-center justify-between mt-4">
                <span className="text-[9px] font-black text-blue-900/30 uppercase tracking-widest">Health-Care OS</span>
                <div className="flex items-center gap-1">
                  <span className="w-1.5 h-1.5 bg-[#2F357D] rounded-full animate-ping"></span>
                  <span className="text-[9px] font-bold text-blue">Syncing</span>
                </div>
              </div>
            </div>
          </div>
     ...
     {/*  LOGOUT  */}
      ...
      `}</style>
    </div> // Ye file ka sabse akhri div hai
  );
};
